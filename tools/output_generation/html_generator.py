...
Refactored to remove redundancies and unify preverb handling
"""
...
class HTMLGenerator:
    ...
    def generate_html(
        ...
        [REFACTORED - Now uses template orchestrator for page structure]
        ...
        complete_html = template_orchestrator.generate_complete_page(
            toc_content=toc_content, verb_sections_html=verb_sections_html
        )
    ...
    def _create_single_preverb_static_section_from_processed_data(
        ...
        # For multi-preverb verbs, use the default preverb
        has_multiple_preverbs = verb.get("preverb_config", {}).get(
            "has_multiple_preverbs", False
        )
    ...
    def _format_georgian_components(self, components: Dict) -> str:
        """Format Georgian text with rich component highlighting and semantic markup."""
        try:
            if not components:
                return ""
            ...
            if "verb" in components:
                formatted_parts.append(self._format_verb_component(components["verb"]))
            if "subject" in components:
                formatted_parts.append(
                    self._format_subject_component(components["subject"])
                )
            if "direct_object" in components:
                formatted_parts.append(
                    self._format_direct_object_component(components["direct_object"])
                )
            if "indirect_object" in components:
                formatted_parts.append(
                    self._format_indirect_object_component(
                        components["indirect_object"]
                    )
                )
    ...
    def _format_english_components(self, components: Dict) -> str:
        """Format English text with rich component highlighting and semantic markup."""
        try:
            if not components:
                return ""

            # Build the formatted text by processing each component
            formatted_parts = []

            # Process subject component
            if "subject" in components:
                subject_data = components["subject"]
                subject_text = subject_data.get("text", "")
                subject_role = subject_data.get("role", "subject")
                subject_person = subject_data.get("person", "")

                person_attr = (
                    f' data-person="{subject_person}"' if subject_person else ""
                )

                formatted_parts.append(
                    f'<span class="component-{subject_role} gloss-subject" data-role="{subject_role}"{person_attr}>{subject_text}</span>'
                )

            # Process verb component
            if "verb" in components:
                verb_data = components["verb"]
                verb_text = verb_data.get("text", "")
                verb_role = verb_data.get("role", "verb")
                formatted_parts.append(
                    f'<span class="component-{verb_role} gloss-verb" data-role="{verb_role}">{verb_text}</span>'
                )

            # Process direct object component
            if "direct_object" in components:
                do_data = components["direct_object"]
                do_text = do_data.get("text", "")
                do_role = do_data.get("role", "direct_object")
                formatted_parts.append(
                    f'<span class="component-{do_role} gloss-direct-object" data-role="{do_role}">{do_text}</span>'
                )

            # Process indirect object component
            if "indirect_object" in components:
                io_data = components["indirect_object"]
                io_text = io_data.get("text", "")
                io_role = io_data.get("role", "indirect_object")
                formatted_parts.append(
                    f'<span class="component-{io_role} gloss-indirect-object" data-role="{io_role}">{io_text}</span>'
                )
    ...
    def _generate_expanded_gloss_section(self, expanded_components: List[Dict]) -> str:
        ...
        for component in expanded_components:
            ...
            if (
                ...
                colored_text = "".join(colored_parts)  # No spaces between components
    ...
    def generate_verb_sections_html(
        ...
        """
        Generate verb sections HTML content.
        Focuses only on verb content generation, not page structure.
        """
        # Get base verbs from processed data manager
        try:
            # Load processed verbs to get base verb information
            processed_verbs_data = self.processed_data_manager.load_processed_verbs()

            base_verbs = []

            # Extract base_data from each processed verb entry
            for verb_id, processed_verb in processed_verbs_data.items():
                base_verb = processed_verb.get("base_data", {})
                if base_verb:
                    base_verbs.append(base_verb)
                else:
                    logger.warning(f"No base_data found for verb {verb_id}")

            logger.info(f"Found {len(base_verbs)} base verbs from processed data")
        except Exception as e:
            logger.error(f"Error loading processed data: {e}")
            # Fallback: create minimal verb data from processed verbs parameter
            logger.info("Using fallback verb data creation")
            base_verbs = []
        ...
        for i, verb in enumerate(base_verbs, 1):
            verb_id = verb.get("id", "")
            if str(verb_id) in processed_verbs:
                processed_verb = processed_verbs[str(verb_id)]
                verb_section = self.create_static_verb_section_from_processed_data(
                    verb, processed_verb, i, duplicate_primary_verbs
                )
                verb_sections_html += verb_section
            else:
                logger.warning(f"No processed data found for verb {verb_id}")

        return verb_sections_html
