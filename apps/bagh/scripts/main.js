...
class App {
    ...
    async initialize() {
        try {
            ...
            window.app = this;
            this.addLocalDevMorphologyEditorLink();
    ...
    /**
     * Add a local-only quick link to morphology editor for developer workflow.
     * This never appears on deployed GitHub pages.
     */
    addLocalDevMorphologyEditorLink() {
        const isLocalDevHost = window.location.hostname === '127.0.0.1' && window.location.port === '8000';
        const isBaghDistIndex = window.location.pathname.endsWith('/dist/index.html') || window.location.pathname === '/dist/index.html';
        if (!isLocalDevHost || !isBaghDistIndex) {
            return;
        }

        if (document.getElementById('local-dev-morphology-link')) {
            return;
        }

        const link = document.createElement('a');
        link.id = 'local-dev-morphology-link';
        link.href = '/apps/morphology-chart/index.html';
        link.textContent = 'Open Morphology Chart Editor';
        link.title = 'Developer shortcut';
        link.style.position = 'fixed';
        link.style.left = '50%';
        link.style.transform = 'translateX(-50%)';
        link.style.bottom = '12px';
        link.style.zIndex = '1100';
        link.style.padding = '8px 12px';
        link.style.borderRadius = '8px';
        link.style.background = '#1f6feb';
        link.style.color = '#ffffff';
        link.style.fontSize = '13px';
        link.style.fontWeight = '600';
        link.style.textDecoration = 'none';
        link.style.boxShadow = '0 2px 8px rgba(0,0,0,0.22)';
